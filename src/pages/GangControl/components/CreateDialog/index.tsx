...
import {createRuleTask, engineType, triggerRules} from "@/http/api/editor";
const weekOptions = [
  {
    label:"星期日",
    value:1
...
const CreateDialog: React.FC<DialogProps> = ({onUpdate,onClose,visible}) => {
  ...
  const handleSubmit = ()=>{
    formRef.validateFields().then((values)=>{
      ...
      week = week?.join(',')
      const params = {...values,startTime,endTime,week}
      createRuleTask(params).then(()=>{
  ...
  useEffect(()=>{
    if (visible) {
      getOptions()
      setCurrentRule(602)
      setCurrentExecutionMode(1)
    }
  },[visible])
  // @ts-ignore
  return <Modal open={visible} title="新增" width={840} onCancel={handleClose} onOk={handleSubmit}>
    <div className="dialog-content">
      <Form labelCol={{span:3}} form={formRef} initialValues={{
        executionMode:1,
        triggerRuleId:602
      }}>
        <Row>
